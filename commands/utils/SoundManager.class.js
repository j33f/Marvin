const { joinVocalChannel, getVolume } = require('./vocChan');
...
class SoundManager {
  constructor({ channelName, interaction, defaultVolumes }) {
  ...
  async play(interaction, soundName, volume = undefined) {
    ...
    const filePath = join(__dirname, '..', 'dnd', 'ambiances', soundName + '.webm');
    const options = {
  ...
  static async getSoundsNames() {
    if (globalSoundNames.length > 0) {
      this.soundNames = globalSoundNames;
      return this.soundNames;
    }

    const files = await glob(join(__dirname, '..', 'dnd', 'ambiances', '*.webm'));
